...
@section('content')
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Manage your library books and inventory.</p>
        </div>
    ...
    <div class="mt-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <form action="{{ route('books.index') }}" method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div class="flex items-end gap-2 md:col-span-3">
                <button type="submit"
                    class="rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-gray-800">Search</button>
                @if (request('search') || request('category_id'))
                    <a href="{{ route('books.index') }}"
                        class="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">Clear</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Book
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">ISBN
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                            Category</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                            Inventory</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Shelf
                        </th>
                    ...
                    @forelse ($books as $book)
                        <tr class="transition hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="mt-1 text-sm text-gray-500">by {{ $book->author }}</div>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600">{{ $book->isbn }}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $book->available_quantity }} available
                                </div>
                                <div class="mt-1 text-xs text-gray-500">{{ $book->quantity }} total</div>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600">
                                {{ $book->shelf_location ?: '—' }}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('books.destroy', $book) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="rounded-lg px-3 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50">Delete</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <h3 class="mt-4 text-sm font-semibold text-gray-900">No books found</h3>
                                <p class="mt-1 text-sm text-gray-500">Get started by adding your first book.</p>
                                <button type="button" onclick="openCreateModal()"
                                    class="mt-4 text-sm font-semibold text-indigo-600 hover:text-indigo-700">Add a
                                    book</button>
                            </td>
                        </tr>
                    @endforelse
